...
function App() {
  ...
  const [empresas, setEmpresas] = useState(() => {
    ...
    return salvos ? JSON.parse(salvos) : [
      { id: 1, nome: "0001 Sitio Matriz", tarefas: {} },
      { id: 2, nome: "0002 Goianinha", tarefas: {} },
      { id: 3, nome: "0003 Nova Cruz", tarefas: {} },
      { id: 4, nome: "0004 São Paulo Potengi", tarefas: {} },
      { id: 5, nome: "0005 Passa e Fica", tarefas: {} },
      { id: 6, nome: "0006 Brejinho", tarefas: {} },
      { id: 7, nome: "0007 Touros", tarefas: {} }
    ];
  });
  ...
  const getCorStatus = (status) => {
    ...
    if (status === 'Andamento') return 'bg-sky-100 border-sky-500 text-sky-700'; // Sky para diferenciar do Navy
    ...
    return (
      <div className="min-h-screen bg-slate-100 flex items-center justify-center p-4 font-sans">
        <div className="max-w-md w-full bg-white rounded-xl shadow-xl overflow-hidden border border-slate-200">
          <div className="bg-blue-900 p-6 text-center"> {/* NOVO AZUL AQUI */}
          ...
          <form onSubmit={(e) => {
            ...
            <button className="bg-blue-800 hover:bg-blue-900 text-white font-bold py-3 rounded shadow-md transition-colors">ENTRAR</button>
          </form>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-4 relative font-sans">
      ...
      <div className={`fixed top-0 right-0 h-full w-80 bg-white shadow-2xl z-50 transform transition-transform duration-300 ${mostrarHistorico ? 'translate-x-0' : 'translate-x-full'} flex flex-col`}>
        <div className="p-4 bg-blue-900 text-white flex justify-between items-center"> {/* NOVO AZUL AQUI */}
      ...
      <header className="max-w-[1800px] mx-auto mb-6 flex flex-col md:flex-row gap-4 justify-between md:items-center border-b pb-4">
        <div className="flex items-center gap-4">
          <div className="w-14 h-14 rounded-full bg-blue-900 text-white flex items-center justify-center font-black text-xl shadow-lg uppercase">{usuarioLogado.charAt(0)}</div> {/* NOVO AZUL AQUI */}
          <div>
            <h1 className="text-xl font-black text-slate-900 uppercase tracking-tight">Monitor Fiscal <span className="text-blue-800">v1.3</span></h1> {/* AZUL AJUSTADO */}
            <p className="text-slate-500 text-xs font-medium">CheckList Auditoria XML RN</p>
          </div>
        </div>
        ...
        <div className="flex flex-col items-end gap-3 w-full md:w-auto">
          ...
          <div className="flex gap-4 items-center">
            ...
            <button onClick={() => setMostrarHistorico(true)} className="text-xs font-bold text-blue-800 hover:text-blue-950 hover:underline">HISTÓRICO</button>
            <button onClick={() => { setUsuarioLogado(null); localStorage.removeItem('usuarioLogado'); }} className="text-xs font-bold text-red-500 hover:text-red-700 hover:underline">SAIR</button>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-[1800px] mx-auto overflow-hidden rounded-xl border border-slate-200 bg-white shadow-xl">
        <div className="overflow-x-auto scrollbar-thin scrollbar-thumb-slate-200 scrollbar-track-white">
          <table className="w-full border-collapse">
            <thead>
              <tr className="bg-blue-800 text-white text-[10px] uppercase"> {/* AZUL AJUSTADO (Sub-header) */}
                <th rowSpan="2" className="p-3 text-left font-bold border-r border-b border-blue-700/50 bg-blue-800 text-white sticky left-0 z-30 w-[150px]">EMPRESAS</th> {/* AZUL SUPER ESCURO NO STICKY */}
              ...
              <tr className="bg-gray-200 text-black text-[9px] uppercase font-bold border-b border-blue-800"> {/* AZUL AJUSTADO (Sub-header 2) */}
                {categorias.map(cat => cat.filhas.map(f => <th key={`${cat.nome}-${f}`} className="p-1 border-r border-blue-600/50">{f}</th>))}
              </tr>
            </thead>
            <tbody>
              {empresas.filter(e => e.nome.toLowerCase().includes(filtro.toLowerCase())).map((emp) => (
                <tr key={emp.id} className="border-b border-slate-100 hover:bg-blue-50 transition-colors">
                  <td className="p-2 font-bold text-slate-700 border-r border-slate-200 bg-white sticky left-0 z-10 text-[10px] truncate shadow-sm group-hover:bg-blue-50" title={emp.nome}>{emp.nome}</td>
                  {categorias.map(cat => cat.filhas.map(sub => {
                    const info = emp.tarefas[`${cat.nome}-${sub}`] || { status: 'Pendente' };
                    return (
                      <td key={`${cat.nome}-${sub}`} className="p-2 border-r border-slate-100 text-center min-w-[60px]">
                        <select 
                          value={info.status} 
                        ...
                        {info.user && info.status !== 'Pendente' && (
                          <div className="mt-1 text-[7px] text-slate-400 leading-none"><b>{info.user.split(' ')[0]}</b><br/>{info.data}</div>
                        )}
                      </td>
                    );
                  }))}
