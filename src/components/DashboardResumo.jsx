...
export function DashboardResumo({ empresas, categorias, totalTarefasRequeridas, tema }) {
  const stats = useMemo(() => {
    let concluidas = 0;
    let pendentes = 0;
    ...
    return { progresso, empresas100, pendentes, verificar, concluidas, total: empresas.length };
  ...
  return (
    <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto transition-colors duration-300">
      ...
      <div className={`${boxEstilos[tema]} border rounded-xl p-3 shadow-sm flex-1 sm:min-w-[220px] flex flex-col justify-center relative overflow-hidden transition-colors`}>
        <div className={`absolute -right-4 -top-4 w-16 h-16 rounded-full blur-2xl ${tema === 'light' ? 'bg-blue-50' : 'bg-blue-900/20'}`}></div>
        ...
        <div className="flex justify-between items-end mb-2 relative z-10">
          ...
          <span className="text-lg font-black text-blue-500 leading-none">{stats.progresso}%</span>
        </div>
        <div className={`w-full rounded-full h-2 relative z-10 ${tema === 'light' ? 'bg-slate-100' : 'bg-slate-700/50'}`}>
          <div 
      ...
      <div className="flex gap-3 flex-wrap">
        ...
        {/* Empresas 100% */}
        <div className={`${bgBoxVerde[tema]} border rounded-xl px-4 py-2 flex flex-col justify-center min-w-[110px] shadow-sm transition-colors`}>
          <span className={`text-[9px] font-bold uppercase tracking-widest mb-0.5 ${tema === 'light' ? 'text-emerald-600/80' : 'text-emerald-500/80'}`}>Empresas 100%</span>
        ...
        {/* Pendências */}
        <div className={`${bgBoxVermelho[tema]} border rounded-xl px-4 py-2 flex flex-col justify-center min-w-[110px] shadow-sm transition-colors`}>
          <span className={`text-[9px] font-bold uppercase tracking-widest mb-0.5 ${tema === 'light' ? 'text-rose-600/80' : 'text-rose-500/80'}`}>Pendências</span>
          <span className="text-xl font-black">{stats.pendentes}</span>
        </div>

        {/* REVISAR (Aparece apenas se houver itens para revisar) */}
        {stats.verificar > 0 && (
          <div className={`${bgBoxAmarelo[tema]} border rounded-xl px-4 py-2 flex flex-col justify-center min-w-[110px] shadow-sm transition-colors animate-pulse`}>
            <span className={`text-[9px] font-bold uppercase tracking-widest mb-0.5 ${tema === 'light' ? 'text-amber-600/80' : 'text-amber-500/80'}`}>Para Revisar</span>
